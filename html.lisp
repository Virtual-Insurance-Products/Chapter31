...
(defun emit-open-tag (processor tag body-p attributes)
  ...
  (raw-string processor (if (and nil ;; *** I want to suppress this tag minimisation feature - it causes problems
				 *xhtml* (not body-p)) "/>" ">")))
...
(defun emit-close-tag (processor tag body-p)
  (unless (and (or *xhtml* (empty-element-p tag))
	       (not body-p)
	       nil) ;; suppress close tag omission. This occasionally
		    ;; causes problems in HTML which is
		    ;; SGML. bother. Not with many things though I
		    ;; don't think.
    (raw-string processor (format nil "</~(~a~)>" tag)))
  (when (or (paragraph-element-p tag) (block-element-p tag))
    (freshline processor)))
